...
## 👤 David Misael Alcocer Castilla: Árbol de Huffman y Generación del Diccionario
...
## 👤 Rodrigo Israel Salazar Perez: Manejo de Bits y Cabecera de Archivos (Header)
...
**Tareas:**
- [x] Implementar función `escribir_bit()` con un buffer `unsigned char` y contadores de turnos lógicos.
- [x] Implementar función `leer_bit()` para la descompresión.
- [x] Crear la funcionalidad de escritura del buffer remanente (flush) al finalizar.
...
**Tareas:**
- [x] Definir una estructura binaria o de texto que almacene las frecuencias directamente en la primera sección del archivo de salida.
- [x] Implementar la función de extracción que construya la misma lista enlazada base a partir de dicho header.

---

## 👤 Leonardo Ariel San Martin Lopez : Lógica de Compresión y Descompresión (Core)
...
**Tareas:**
- [x] Invocar la escritura del Header (creado por el Miembro 2).
- [x] Recorrer el archivo `.txt` original por segunda vez carácter a carácter.
- [x] Buscar en la `TablaCodigos` y enviar la secuencia individual al módulo de escritura de `bits`.
- [x] Cerrar y limpiar canales de datos.
...
**Tareas:**
- [x] Leer el Header y rearmar la raíz del Árbol de Huffman (apoyado en Miembro 1).
- [x] Leer el archivo a bits usando las utilidades del Miembro 2 y navegar sobre el Árbol desde la raíz a las hojas por cada `0/1` hallado.
- [x] Al llegar a la hoja, imprimir el carácter original en el archivo `descomprimido.txt`.
- [x] Cargar validaciones de alcance final (padding/bits basura).
